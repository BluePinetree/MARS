"""
Local context store for compact run memory (LangGraph prototype).
"""
...
class LocalContextStore:
    """Manage shared memory and run-scoped compact context files."""
    ...
    def __init__(self, run_output_dir: Path, project_root: Path):
        ...
        if not self.runtime_memory_path.exists():
            self.runtime_memory_path.write_text("# Runtime Context\n\n", encoding="utf-8")
        if not self.handoff_state_path.exists():
            self.handoff_state_path.write_text(json.dumps({}, ensure_ascii=False, indent=2), encoding="utf-8")
        if not self.compact_history_path.exists():
            self.compact_history_path.write_text("# Compact History\n\n", encoding="utf-8")
